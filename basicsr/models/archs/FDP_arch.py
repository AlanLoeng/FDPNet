...
from basicsr.models.pnps import FrequencyDomainProcessing
...
class FDPBlock(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        super().__init__()
        dw_channel = c * DW_Expand // 2
        self.conv1 = nn.Conv2d(in_channels=c, out_channels=dw_channel, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        self.conv2 = nn.Conv2d(in_channels=dw_channel, out_channels=dw_channel, kernel_size=3, padding=1, stride=1, groups=dw_channel,
                               bias=True)
        # 频域特征处理
        self.fdp = FrequencyDomainProcessing.FrequencyDomainProcessing(in_channels=dw_channel)
...
class FDP(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        chan = width
        for layer_idx, num in enumerate(enc_blk_nums):
            self.encoders.append(
                nn.Sequential(
                    *[
                        FDPBlock(chan) if (layer_idx + i) % 2 != 0 else NAFBlock0(chan)
                        for i in range(num)
                    ]
                )
            )
...
class FDPLocal(Local_Base, FDP):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        FDP.__init__(self, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    net = FDP(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
