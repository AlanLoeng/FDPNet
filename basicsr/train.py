...
def parse_options(is_train=True):
    ...
    # The 'use_amp' option is now expected to be in the YAML file
    # Default to False if not specified
    if 'use_amp' not in opt:
        opt['use_amp'] = False
...
def init_loggers(opt):
    ...
    if opt.get('use_amp', False):
        logger.info('Automatic Mixed Precision (AMP) training is enabled')
...
def main():
    ...
    # if opt.get('use_amp', False) and not torch.cuda.is_available():
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            iter_time = time.time()
            train_data = prefetcher.next()
            # epoch = current_iter // num_iter_per_epoch
            #when (num of img) % batch_size == 0 , train_data will never be None
            # if current_iter % num_iter_per_epoch == 0:
            #     break
        # end of iter
        epoch += 1
